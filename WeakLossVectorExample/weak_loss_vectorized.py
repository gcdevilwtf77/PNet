...
odes = [(F1,y1,2,'F1',0.01,10000),(F2,y2,2,'F2',0.01,10000),(F3,y3,2,'F3',0.01,10000),(F4,y4,2,'F4',0.01,10000),
        ...
        (F7,y7,torch.pi,'F7_3',0.01,1000000), (F7,y7,torch.pi,'F7_4',0.01,10000000),
        (F8,y8,torch.pi,'F8',0.01,10000)] #List of (ODE,True solution,Final time,name)

#Loop over ODEs to train on
for (F,y,T,name,lr,epochs) in odes[-1:]:
    utils_vectorized.train(F,y(zero),T,lr=lr,epochs=epochs,savefile=name+'_model.pt')
    utils_vectorized.plot(F,y(zero),T,y,model_name=name+'_model.pt',filename_prefix=name)
